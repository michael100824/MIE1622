...
for (period = 1:N_periods)
   ...
   for(strategy = 1:N_strat)
      ...
      if strategy == 6
        portf_value{strategy}(day_ind_start:day_ind_end) = data_prices(day_ind_start:day_ind_end,:) * x{strategy,period} + cash{strategy,period} - 1000000; %r_rf/12 * period;
      else
...
xlabel('Trading Days');
ylabel('Value');
title('Daily Value of Portfolio (2015-2016)')
...
%%
% plot strategy 7 RMV to show dynamic changes in portfolio allocations
...
ylabel('Weight');
title('Dynamic Change in Portfolio Alloction - Robust Mean Variance (2015-2016)');
...
y_axis = zeros(N_periods,20);
for i = 1:N_periods % for 12 period
    minvar_pos = x{3,i}; % select weight info for strategy 7
    index = 42 * (i - 1) + 1;
    value_of_each_stock = minvar_pos' .* data_prices(index,:);
...
xlabel('Trading Period');
ylabel('Weight');
title('Dynamic Change in Portfolio Alloction - Mean Variance (2015-2016)');
legend('MSFT', 'F',	'CRAY', 'GOOG',	'HPQ','YHOO','HOG','VZ','AAPL',...
    'IBM','T','CSCO','BAC','INTC','AMD','SNE','NVDA','AMZN','MS','BK');
%%
y_axis = zeros(N_periods,20);
...
xlabel('Trading Period');
ylabel('Weight');
title('Dynamic Change in Portfolio Alloction - Max Sharpe Ratio (2015-2016)');
legend('MSFT', 'F',	'CRAY', 'GOOG',	'HPQ','YHOO','HOG','VZ','AAPL',...
    'IBM','T','CSCO','BAC','INTC','AMD','SNE','NVDA','AMZN','MS','BK');
